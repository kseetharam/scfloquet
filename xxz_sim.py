...
if __name__ == "__main__":
  ...
  print(J_eff, Delta_eff)

  tscale = 1 / J_eff  # time scale of effective dynamics in microseconds
  tmax = 10 * tscale; dt = tscale/10
  tgrid1 = np.arange(0, tmax, dt)  # time grid in microseconds
  tgrid2 = tscale*np.logspace(1,4,100)
  tgrid = np.concatenate((tgrid1, tgrid2))
  ...
  for n in range(N_d):
    ...
    ds = scfuncs.xxzSim_qT_unitary(tgrid, N, J_eff, Delta_eff, hi_eff_List)
    print(timer() - tstart)
    SFF_mat[n,:] = ds['SFF'].values

    tstart = timer()
    # ds_aveHam = scfuncs.xxzSim_qT_aveHam(tgrid, N, J_S, J_I, hiList, Delta_eff, kappa, gamma_amp, gamma_phase)
    ds_aveHam = scfuncs.xxzSim_qT_aveHam_unitary(tgrid, N, J_S, J_I, hiList, Delta_eff)
    print(timer() - tstart)
  ...
  np.save(datapath + 'SFF_unitary',SFF)
  np.save(datapath + 'SFF_aveHam_unitary',SFF_aveHam)

  # SFF = np.load(datapath + 'SFF_unitary.npy')
  # SFF_aveHam = np.load(datapath + 'SFF_aveHam_unitary.npy')
